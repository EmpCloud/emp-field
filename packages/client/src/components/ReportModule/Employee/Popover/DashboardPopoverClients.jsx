...
const DashboardPopoverClients = ({ exportPDFclientReport, client }) => {
  const headers = [['Client Name', 'Phone Number']];
  const xlsvHeaders = ['Client Name', 'Phone Number'];
  return (
    <Popover.Root>
        ...
        <div className="filter_content_container select-none  bg-white card-shadow w-full h-full rounded-sm flex flex-col items-center justify-start mt-1 ">
          ...
          <Popover.Close asChild>
            <div
              ...
              onClick={() => {
                exportCSV(client, headers, 'Client Report');
              }}>
              <img
                src={CSVIconImage}
                className="w-[14px] 2xl:w-4 2xl:h-4"
          ...
          <Popover.Close asChild>
            <div
              className="export-csv flex justify-center items-center rounded-sm gap-2 px-3 py-2 font-medium hover:bg-slate-100 cursor-pointer"
              onClick={() => {
                exportXLS(client, xlsvHeaders, 'Client Report');
              }}>
              <img
                src={XLSIconImage}
                className="w-[14px] 2xl:w-4 2xl:h-4"
...
export default DashboardPopoverClients;
