...
const NAV_ITEMS: NavItem[] = [
  ...
  { to: "/client-sites", label: "Client Sites", icon: Building2 },
    ...
    return (
      <div className="flex h-full w-64 flex-col bg-white border-r border-gray-200">
        ...
        <div className="flex h-16 items-center gap-3 px-6 border-b border-gray-100">
          ...
          <span className="text-lg font-bold text-gray-900">EMP Field</span>
        </div>

        {/* Nav */}
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          {NAV_ITEMS.filter((item) => {
            if (item.adminOnly && !ADMIN_ROLES.includes((user?.role || "employee") as Role))
              return false;
            return true;
          }).map((item) => (
            <NavLink
